#include <stdint.h>
#include <memoryUnit.h>
#include <cpu.h>
#include <gpu.h>

MemoryUnit *MMU;

MemoryUnit *initMMU()
{
    MemoryUnit *p = malloc(sizeof(MemoryUnit));
    p->inbios = false;
    return p;
}

void initBios(){
    unsigned char bios[] = {};

    for (int i = 0; i < 256; i++)
    {
        MMU->bios[i] = bios[i];
    }
}

char rb(uint16_t addr)
{
    switch (addr & 0xF000)
    {
    case 0x0000:
        /* code */
        if (MMU->inbios)
        {
            if (addr < 0x0100)
            {
                return MMU->bios[addr];
            }
            else if (Z80->sp == 0x100)
            {
                MMU->inbios = true;
            }
        }
        return MMU->rom[addr - 0x100];

    // ROM 0
    case 0x1000:
    case 0x2000:
    case 0x3000:
    // ROM 1 unbanked
    case 0x4000:
    case 0x5000:
    case 0x6000:
    case 0x7000:
        return MMU->rom[addr - 0x100];

    case 0x8000:
    case 0x9000:
        return MMU->vram[addr & 0x1FFF];

    case 0xA000:
    case 0xB000:
        return MMU->eram[addr & 0x1FFF];

    case 0xC000:
    case 0xD000:
        return MMU->wram[addr & 0x1FFF];
    case 0xE000:
        return MMU->wshadow[addr & 0x1FFF];
    case 0xF000:
        switch (addr & 0x0F00)
        {
        case 0x000:
        case 0x100:
        case 0x200:
        case 0x300:
        case 0x400:
        case 0x500:
        case 0x600:
        case 0x700:
        case 0x800:
        case 0x900:
        case 0xA00:
        case 0xB00:
        case 0xC00:
        case 0xD00:
            return MMU->wshadow[addr & 0x1FFF];
        case 0xF00:
            if (addr >= 0xFF80)
            {
                return MMU->zram[addr & 0x7F];
            }
            else
            {
                // I/O control handling
                return MMU->mmio[addr & 0x7F];
                // Currently unhandled
            }
        default:
            return -1;
        }
    default:
        return -1;
    }
}

short rw(uint16_t addr)
{
    return rb(addr) + (rb(addr + 1) << 8);
}

void wb(uint16_t addr, unsigned char val)
{

    switch (addr & 0xF000)
    {
        case 0x0000:
            if (addr < 0x0100)
            {
                MMU->bios[addr] = val;
            }else{
                MMU->rom[addr - 0x100] = val;
            }
            break;
        case 0x1000:
        case 0x2000:
        case 0x3000:
        case 0x4000:
        case 0x5000:
        case 0x6000:
        case 0x7000:
            MMU->rom[addr - 0x100] = val;
            break;
        case 0x8000:
        case 0x9000:
            MMU->vram[addr & 0x1FFF] = val;
            updateTile(addr, val);
            break;

        case 0xA000:
        case 0xB000:
            MMU->eram[addr & 0x1FFF] = val;
            break;

        case 0xC000:
        case 0xD000:
            MMU->wram[addr & 0x1FFF] = val;
            break;
        case 0xE000:
            MMU->wshadow[addr & 0x1FFF] = val;
            break;
        case 0xF000:
            if (addr >= 0xFF80){
                MMU->zram[addr & 0x7F] = val;
            }else{
                MMU->mmio[addr & 0x7F] = val;
                switch (addr & 0x00F0)
                {
                    case 0x40: case 0x50: case 0x60: case 0x70:
                        updateGpuRegisters(addr, val);
                        break;
                    default:
                        break;
                } 
            }
            break;
        default:
            break;
    }
}

void ww(uint16_t add, unsigned char val[2])
{
    wb(add, val[0]);
    wb(add + 1, val[1]);
}